...
class DecisionTreeClassificationPipeline(A.Pipeline):
    def __init__(self):
    ...
    def run(self, fetch_data=False, save_model=False, verbose=True):
        ...
        if save_model:
            self.save_model("dt_classification.pkl")
            self.export_graph("dt_classification.dot")
    ...
    def export_graph(self, path):
        export_graphviz(
            self.model,
            out_file=path,
            feature_names=self._full_data.feature_names[2:],
