...
class DecisionTreeRegressionPipeline(A.Pipeline):
    def __init__(self):
    ...
    def split_data(self, test_ratio=0.8):
        x = self._full_data['data'][:, 3:]
        y = (self._full_data['target'] == 2).astype(np.float64)
    ...
    def run(self, fetch_data=False, save_model=False, verbose=True):
        ...
        if save_model:
            self.save_model("dt_regression.pkl")
            self.export_graph("dt_regression.dot")
    ...
    def export_graph(self, path):
        export_graphviz(
            self.model,
            out_file=path,
            feature_names=self._full_data.feature_names[3:],
            class_names=self._full_data.target_names,
            rounded=True,
            filled=True
        )
