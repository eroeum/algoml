...
class NonlinearSVMPipeline(A.Pipeline):
    def __init__(self):
        ...
        self.path = os.path.join("datasets", "moon")
        self.name_csv = 'moon.csv'
    ...
    def fetch_data(self, url=None, path=None, name=None):
        ...
        if name is None:
            name = self.name_csv
        moons = datasets.make_moons(random_state=42)
        df = pd.DataFrame(moons[0])
        df['labels'] = moons[1]
        df = A.pandas_to_csv(df, path=path, name=name)
        return df

    def ingest_data(self, path=None, name=None):
        self._full_data = datasets.make_moons(random_state=42)
    ...
    def run(self, fetch_data=False, save_model=False, verbose=True):
        ...
        if save_model:
            self.save_model("nonlinear_svm.pkl")
